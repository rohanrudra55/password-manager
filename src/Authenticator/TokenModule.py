...
class Token:
    ...
    def __init__(self,assets_path):
        self.message_template_path = assets_path+"/assets/email_body_"
    ...
    @staticmethod
    def __generate_otp(length=6):
        otp = ''.join([str(random.randint(0, 9)) for _ in range(length)])
        return otp

    def set_template(self, id_input):
    ...
    def verify(self, recipient_email, token):
        placeholder = token // 10
        key = token % 10
        otp = self.__generate_otp(5)
        self.set_template('cls6789678')
        self.send_mail(otp, recipient_email)
